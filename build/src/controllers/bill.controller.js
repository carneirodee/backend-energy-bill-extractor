...
export default class BillController {
    ...
    getByClientCode = async (req, res, next) => {
        let id = req.params.id;
        try {
    ...
    getBillsByClients = async (req, res, next) => {
        try {
    ...
    getAllDashboard = async (req, res, next) => {
        try {
            const data_bills = await this.repository.getAll();
            let total_cons = 0;
            ...
            let total_s_gd = 0;
            let total_eco_gd = 0;
            let energy_qt_vs_compensated = [];
            let value_w_gd_vs_eco_gd = [];
            ;
            data_bills.map((bill) => {
            ...
            const result1 = energy_qt_vs_compensated.reduce((acc, curr) => {
                const objInAcc = acc.find((o) => o.month === curr.month);
                if (objInAcc) {
            ...
            const result2 = value_w_gd_vs_eco_gd.reduce((acc, curr) => {
                const objInAcc = acc.find((o) => o.month === curr.month);
                if (objInAcc) {
